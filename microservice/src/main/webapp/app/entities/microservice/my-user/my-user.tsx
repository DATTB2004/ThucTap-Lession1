...
export const MyUser = () => {
  ...
  const myUserList = useAppSelector(state => state.microservice.myUser.entities);
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('idUser')}>
                  Id User <FontAwesomeIcon icon={getSortIconByFieldName('idUser')} />
                </th>
            ...
            <tbody>
              {myUserList.map((myUser, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/microservice/my-user/${myUser.id}`} color="link" size="sm">
                      {myUser.id}
                    </Button>
                  </td>
                  <td>{myUser.idUser}</td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`/microservice/my-user/${myUser.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button
                        tag={Link}
                        to={`/microservice/my-user/${myUser.id}/edit`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() => (window.location.href = `/microservice/my-user/${myUser.id}/delete`)}
                        color="danger"
                        size="sm"
