...
import static com.bksoft.domain.AttributeAsserts.*;
...
class AttributeResourceIT {
    ...
    public static Attribute createEntity() {
        return new Attribute().name(DEFAULT_NAME);
    }
    ...
    public static Attribute createUpdatedEntity() {
        return new Attribute().name(UPDATED_NAME);
    }
    ...
    void createAttribute() throws Exception {
        ...
        var returnedAttribute = om.readValue(
            restAttributeMockMvc
                .perform(post(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(attribute)))
    ...
    void putExistingAttribute() throws Exception {
        ...
        updatedAttribute.name(UPDATED_NAME);
    ...
    void putNonExistingAttribute() throws Exception {
        ...
        restAttributeMockMvc
            .perform(
                put(ENTITY_API_URL_ID, attribute.getId()).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(attribute))
            )
            .andExpect(status().isBadRequest());
    ...
    void putWithMissingIdPathParamAttribute() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAttributeMockMvc
            .perform(put(ENTITY_API_URL).contentType(MediaType.APPLICATION_JSON).content(om.writeValueAsBytes(attribute)))
    ...
    void partialUpdateAttributeWithPatch() throws Exception {
        ...
        partialUpdatedAttribute.name(UPDATED_NAME);
    ...
    void patchWithMissingIdPathParamAttribute() throws Exception {
        ...
        // If url ID doesn't match entity ID, it will throw BadRequestAlertException
        restAttributeMockMvc
            .perform(patch(ENTITY_API_URL).contentType("application/merge-patch+json").content(om.writeValueAsBytes(attribute)))
            .andExpect(status().isMethodNotAllowed());
